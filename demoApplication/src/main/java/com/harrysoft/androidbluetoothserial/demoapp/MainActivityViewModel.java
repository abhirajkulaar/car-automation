...
public class MainActivityViewModel extends AndroidViewModel {
    ...
    private AudioManager audioLocal;
    ...
    public boolean setupViewModel(String deviceName, String mac, AudioManager audio, IntConsumer  lambda) {

        audioLocal = audio;
        // Check we haven't already been called
        LaunchAssistantFunc = lambda;
        if (!viewModelSetup) {
            viewModelSetup = true;
    ...
    private void onMessageReceived(String message) {
       // Toast.makeText(getApplication(), "message received", Toast.LENGTH_LONG).show();
        ...
        if(message.contains("INC_VOL"))
        {
            System.out.println(messages.toString());
            audioLocal.adjustStreamVolume(AudioManager.STREAM_MUSIC,
                    AudioManager.ADJUST_RAISE, AudioManager.FLAG_SHOW_UI);
        }
        else if(message.contains("DEC_VOL"))
        {
            audioLocal.adjustStreamVolume(AudioManager.STREAM_MUSIC,
                    AudioManager.ADJUST_LOWER, AudioManager.FLAG_SHOW_UI);
        }
                if(message.contains("PLAY_PAUSE"))
        {
            if(audioLocal.isMusicActive())
            {
                KeyEvent event = new KeyEvent(KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_MEDIA_PAUSE);
                audioLocal.dispatchMediaKeyEvent(event);
            }
            else{
        ...
        if(message.contains("TRACK_PREV"))
        {
            KeyEvent event = new KeyEvent(KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_MEDIA_PREVIOUS);
            audioLocal.dispatchMediaKeyEvent(event);
        }
        if(message.contains("TRACK_NEXT"))
        {
        ...
        if(message.contains("ASSISTANT"))
        {
            //KeyEvent event = new KeyEvent(KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_MEDIA_NEXT);
            //audioLocal.dispatchMediaKeyEvent(event);
            LaunchAssistantFunc.accept(1);

           // communicateActivity.LaunchAssistant();
        }
    ...
    private void onMessageSent(String message) {
        ...
        Toast.makeText(getApplication(), "Sent message", Toast.LENGTH_LONG).show();
    ...
    public void sendMessage(String message) {
       ...
       // Toast.makeText(getApplication(), "Sending message", Toast.LENGTH_LONG).show();
    }
